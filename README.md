...
- 🧭 **可切換主題** — 預設 `embodied_ai`，亦可選擇 `audio_speech`、`general_ai` 或自訂 profile
...
```text
使用 daily-paper-scout skill，讀取最新發布日的 JSON 並產生研究報告。
```
...
| `source_limits.arxiv_category_max_results` | 每個 arXiv 分類最多讀取幾篇候選 | `100` |
| `source_limits.arxiv_keyword_max_results` | 每個 arXiv 搜尋詞最多讀取幾篇候選 | `50` |
