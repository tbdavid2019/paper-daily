...
Use this skill when working with the `tbdavid2019/paper-daily` dataset or a deployment of the same schema. It is a discovery and ranking workflow: the JSON is a curated metadata feed, while full-paper reading is a supplemental research step.
...
Use a repository or base URL supplied by the user. Use a date supplied by the user. With neither value, read `index.json` and use its `latest` entry. Treat the dataset date as authoritative for the report.
...
Use the host's normal HTTP, web, or file-reading tool. Reserve repository cloning for local development requests.

## Interpretation rules

- The file `date` is the discovery report date; each paper keeps its own submission metadata.
...
- `published_at` is upstream publication/submission metadata; peer review, acceptance, and daily inclusion each have separate meanings.
- `priority` is a crawler pre-ranking; re-score against the current researcher profile.
...
- `abstract` may be truncated. Base experiments, numbers, conclusions, and claims on the fetched material.
- A paper appearing in multiple sources is a discovery signal; quality assessment requires paper evidence.
...
- If the date has zero selected papers, state that result plainly and keep the report within the selected date window. Broaden the window when the user requests it.
...
4. **Interesting** — plausible ideas with weaker direct fit; preserve calibrated scores.
...
When more detail is needed, follow the paper URL and read the full arXiv abstract, paper, code repository, or discussion. Label these as supplemental sources and keep them separate from the daily JSON.

## Failure handling

- If `index.json` cannot be fetched, report the URL and keep the report date unresolved.
- If the selected date file is missing, report the available dates from the index.
- If a source count is zero, treat it as a source outage or best-effort limitation; global paper availability requires broader evidence.
...
Repository data, topic profiles, and researcher profiles remain read-only during analysis. Apply modifications when the user explicitly requests a code or configuration change.
